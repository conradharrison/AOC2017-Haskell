...
main = do
        ...
        print $ let ilist = map (\s -> read s :: Int) (words instr)
                    initState = ([], ilist, 0)
                    (_, l, _) = run initState -- run once to hit repetition
                    (_, _, c) = run ([], l, 0) in -- run again to measure next interval. Inefficient
                 c
